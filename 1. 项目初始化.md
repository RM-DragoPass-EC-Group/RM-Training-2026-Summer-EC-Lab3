...
RoboMaster C 板使用的芯片型号为 `STM32F407IGH6`。但是本文以 `STM32F401CCU6` 为例，进行演示。
...
用 VS Code 打开 CubeMX 项目所在目录。

打开 EIDE 侧边栏，选择“新建项目” - “空项目” - “Cotex-M 项目”。项目名称需与之前的 CubeMX 工程名称一致；选择项目保存位置时，需选择 CubeMX 项目的**上一层**目录：
...
- 在项目资源中添加文件夹时，如果选择“普通文件夹”，EIDE 就会自动递归地包含子文件夹，而不用手动把每个文件都添加进去；
- 添加 `.s` 汇编文件时，需在 Windows 文件对话窗口中选择 asm 文件类型，否则可能无法显示 `.s` 文件；
- 实际的文件名、目录路径等，根据之前选择的 MCU 型号和 CubeMX 工程配置而定，可能与下图不同。

<img src="./assets/project-init/9.png" alt="配置 EIDE 项目" width="400"/>
<br/>

除了“烧录配置”以外，其他配置项都可以在 CubeMX 生成的 `Makefile` 文件中找到对应的内容。你可以询问 AI，如何根据 `Makefile` 的内容配置 EIDE。

配置完成后，观察一下 Git 仓库的状态：
...
然后配置时钟树（下图以 `STM32F401CCU6` 核心板为例）：

<img src="./assets/project-init/12.png" alt="配置时钟树" width="600"/>

Resolve Clock Issues 之后，注意对比一下是否和图中的配置一致。

对于 RoboMaster C 板（`STM32F407IGHx`），时钟树配置如下：

<img src="./assets/project-init/RM-Type-C-clk-tree.png" alt="RoboMaster C 板时钟树配置" width="600"/>
